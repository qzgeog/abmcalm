...
### To fix all attributes of agents ###

### define class: Plot (Determined) ###
class Plot():
    """ Agent Class: Plot (determined) """
    
    ## define plot features ##
    def __init__(self, plid, centerX, centerY, area, code, dry, elev, slope, 
                 aspect, twi, distCcfp, distEwfp, distEwfpO, 
                 hhid, rgid, villid, geoDist, ifCcfp, ifCcfpO):
        ...
        # continue with linked household
        self.hhid     =   int(hhid)
        self.rgid     =   int(rgid)
        self.villid   =   int(villid)
        self.geoDist  = float(geoDist)
        self.ifCcfp   =   int(ifCcfp)
        self.ifCcfpO  =   int(ifCcfpO)
        # generate point
        self.plPnt    = Point(self.centerX, self.centerY)
        # initiate abandonment year for abandoned plots
        ...
        self.abanYr = [-99,custm_abanYr.rvs()][self.code==69]  # customized pmf
    # make ids private
    @property
    def plid(self):
        return self.__plid
...
### define class: Household (Detemrined) ###
class Household():
    """ Agent Class: Household (determined) """
    
    ## define hh characteristics ##
    def __init__(self, hhid, rgid, villid, hhLocX, hhLocY, hhElev, 
                 hhLandOwn, hhLandPlant, areaCcfp, ifCcfp, ifCcfpO):
        self.hhid        =   int(hhid)
        self.__rgid      =   int(rgid)
        self.__villid    =   int(villid)
        self.hhLocX      = float(hhLocX)
        self.hhLocY      = float(hhLocY)
        self.hhElev      = float(hhElev)
        self.hhLandOwn   = float(hhLandOwn)
        self.hhLandPlant = float(hhLandPlant)
        self.areaCcfp    = float(areaCcfp)
        self.ifCcfp      =   int(ifCcfp)
        self.ifCcfpO     =   int(ifCcfpO)
        # generate point
        self.hhPnt       = Point(self.hhLocX, self.hhLocY)
    
        # initialize EWFP area
        ...
        if self.hhElev < 700:
            ...
            self.areaEwfp=10.0*[custm_ewfp_low.rvs() -rand.random(),0][rvs==0]
        else:
            rvs = custm_ewfp_high.rvs()
            self.areaEwfp=30.0*[custm_ewfp_high.rvs()-rand.random(),0][rvs==0]
...
### define class: Individual (Detemrined) ###
class Individual():
    """ Agent Class: Individual (determined) """
    
    ## define attributes ##
    def __init__(self, pid, hhid, rgid, villid, relateHead, ifFemale, age, 
                 ifMarried, numChild, ifCcfp, ifCcfpO):
        self.__pid      = int(pid)
        self.hhid       = int(hhid)
        self.__rgid     = int(rgid)
        self.__villid   = int(villid)
...
""" Function: Initialize household characteristics (demographic) """
def init_hh_demo(hhList, inList, mu, sig):
    # hhList=aglistHHs, inList=aglistINs, mu=sn1, sig=sn2
    
    ## Household: initialized 7 attributes
        # hdIfFemale, hdAge, hdEdu, hdIfMarried
        # hhSize, hhNumCurOut, hhNumPreOut
