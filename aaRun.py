...
import azagents, aworld, afunc, awrite

# --- Run Time (the whole simulations) --- #
start_time_abm = time.time()
# --- Run Time (the whole simulations) --- #

# send to computer cluster: LongLeaf (UNC)
jobid = sys.argv[1]
# jobid = 1
...
# validation data
fn_vd_in    = 'vdINs.csv'       # individuals with determined attribtues
fn_vd_hh    = 'vdHHs.csv'       # households with determined attribtues
fn_vd_pl    = 'vdPLs.csv'       # households with determined attribtues
...
validate =  0                   # if all attributes are determined
# probabilities
pReturn  =  0.23                # probability of return-migration
pRent    =  0.16                # probability of rent out a plot
...
for tick in range(ticks):   # 15' ~ 18'
    ...
    for agPL in aglistPLs:
        ...
        if agPL.code in [60,61] and afunc.if_qualify_aba(p=agPL): # [afunc.py-T5]
    ...
    for agPL in aglistPLs:
        ...
        if agPL.code in [69] and afunc.if_qualify_aba(p=agPL):
