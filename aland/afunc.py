...
def pred_mig(p, hhList, f1, f2, t, e, fb):
    ...
    y = ((-1.151*p.ifFemale) + (-0.143*p.age) + ( 0.074*p.edu) + 
         (0.502*p.ifMarried) + (-1.241*tHH.hdIfFemale) + ( 0.094*tHH.hdAge) + 
         (-0.048*tHH.hdEdu) + (-0.107*tHH.hhSize) + 
         (0.1355*tHH.hhNumPreOut) + (-0.174*tHH.hhLandOwn) +  
         ( 1.126*payCcfp) + (-0.455*payEwfp) + 0.774)
...
def if_qualify_aba(p, v): # modify (a-land)
    # p=agPL, v=violate
    ...
    # modify (a-land)
    if ifQualified == False:
        ifQualified = [False,True][rand.random()<v]
...
def pred_aban(p, hhList, f1, f2, t, e, fb):
    ...
    y = ((-0.010*area) + (-0.936*dry) + (0.051*geoDist) + (0.035*elev) + 
         (0.026*slope) + (-0.062*dCcfp) + (-0.595*dEwfp) + 
         (0.239*tHH.hdIfFemale) + (-0.009*tHH.hdAge) + 
         (-0.033*tHH.hdEdu) + (0.045*famSize) +
         (-0.3454*payCcfp)  + ( 0.5348*payEwfp) + (-0.467))
    pr = math.exp(y) / (1 + math.exp(y))
